...
class Order(db.Model):
    ...
    place_id = db.relationship(
        "Place", secondary=ASSOCIATION_TABLE, back_populates="order_id"
    )
...
class Place(db.Model):
    ...
    place_type_id = db.Column(
        db.Integer, db.ForeignKey("place_type.id"), nullable=False
    )
    place_type = db.relationship("PlaceType", back_populates="places")
    order_id = db.relationship(
        "Order", secondary=ASSOCIATION_TABLE, back_populates="place_id"
    )
    ...
    def __repr__(self):
        return (
            ...
            " price: {3}".format(
                self.id,
                PlaceType.query.filter_by(id=self.place_type_id)
                .options(load_only("place_type_name"))
                .first(),
                self.place_class,
                self.price,
            )
